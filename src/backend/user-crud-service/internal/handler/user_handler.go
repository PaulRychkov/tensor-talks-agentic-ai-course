...
import (
	...
	"github.com/tensor-talks/user-crud-service/internal/metrics"
...
func (h *UserHandler) RegisterRoutes(router gin.IRouter) {
	...
	router.PATCH("/users/:id/recovery-key-hash", h.SetRecoveryKeyHash)

	// Login word dictionary endpoints (§10.10)
	router.GET("/login-words/adjectives", h.GetActiveAdjectives)
	router.GET("/login-words/nouns", h.GetActiveNouns)
	router.POST("/login-words/generate-random", h.GenerateRandomLogin)
	router.POST("/login-words/check-availability", h.CheckLoginAvailability)
...
// GetActiveAdjectives returns all active adjectives for login generation (§10.10).
func (h *UserHandler) GetActiveAdjectives(c *gin.Context) {
	words, err := h.svc.GetActiveAdjectives(c.Request.Context())
	if err != nil {
		h.logger.Error("GetActiveAdjectives failed", zap.Error(err))
...
// GetActiveNouns returns all active nouns for login generation (§10.10).
func (h *UserHandler) GetActiveNouns(c *gin.Context) {
	words, err := h.svc.GetActiveNouns(c.Request.Context())
	if err != nil {
		h.logger.Error("GetActiveNouns failed", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": "internal error"})
		return
	}
	c.JSON(http.StatusOK, words)
...
// GenerateRandomLogin generates a guaranteed-unique random login (§10.10).
func (h *UserHandler) GenerateRandomLogin(c *gin.Context) {
	login, err := h.svc.GenerateRandomLogin(c.Request.Context())
	if err != nil {
		h.logger.Error("GenerateRandomLogin failed", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to generate login: " + err.Error()})
		return
	}
...
// CheckLoginAvailability checks whether a given login is available (§10.10).
func (h *UserHandler) CheckLoginAvailability(c *gin.Context) {
	var req checkAvailabilityRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "login is required"})
		return
	}
	available, err := h.svc.CheckLoginAvailability(c.Request.Context(), req.Login)
	if err != nil {
		h.logger.Error("CheckLoginAvailability failed", zap.Error(err))
		c.JSON(http.StatusInternalServerError, gin.H{"error": "internal error"})
		return
	}
	c.JSON(http.StatusOK, gin.H{"login": req.Login, "available": available})
}
...
func (h *UserHandler) CreateUser(c *gin.Context) {
	...
	if err != nil {
		...
		case service.ErrInvalidInput:
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "create", "error").Inc()
	...
	metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "create", "success").Inc()
...
func (h *UserHandler) GetUserByID(c *gin.Context) {
	...
	if err != nil {
		if err == repository.ErrNotFound {
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "get_by_id", "not_found").Inc()
			h.logger.Warn("GetUserByID: user not found", zap.String("user_id", externalID.String()))
			c.JSON(http.StatusNotFound, gin.H{"error": "user not found"})
		} else {
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "get_by_id", "error").Inc()
	...
	metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "get_by_id", "success").Inc()
...
func (h *UserHandler) GetUserByLogin(c *gin.Context) {
	...
	if err != nil {
		if err == repository.ErrNotFound {
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "get_by_login", "not_found").Inc()
			h.logger.Warn("GetUserByLogin: user not found", zap.String("login", login))
			c.JSON(http.StatusNotFound, gin.H{"error": "user not found"})
		} else {
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "get_by_login", "error").Inc()
			h.logger.Error("GetUserByLogin failed: internal error", zap.Error(err), zap.String("login", login))
			c.JSON(http.StatusInternalServerError, gin.H{"error": "internal error"})
		}
		return
	}
	metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "get_by_login", "success").Inc()
...
func (h *UserHandler) UpdateUser(c *gin.Context) {
	...
	if err != nil {
		switch err {
		case service.ErrInvalidInput:
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "update", "error").Inc()
		...
		case repository.ErrNotFound:
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "update", "not_found").Inc()
	...
	metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "update", "success").Inc()
...
func (h *UserHandler) DeleteUser(c *gin.Context) {
	...
	if err := h.svc.DeleteUser(c.Request.Context(), externalID); err != nil {
		if err == repository.ErrNotFound {
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "delete", "not_found").Inc()
			h.logger.Warn("DeleteUser: user not found", zap.String("user_id", externalID.String()))
			c.JSON(http.StatusNotFound, gin.H{"error": "user not found"})
		} else {
			metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "delete", "error").Inc()
	...
	metrics.BusinessUserOperationsTotal.WithLabelValues("user-crud-service", "delete", "success").Inc()
...
// SetRecoveryKeyHash обрабатывает PATCH /users/:id/recovery-key-hash.
// Принимает JSON `{ "recovery_key_hash": "..." }` и сохраняет bcrypt-хеш ключа восстановления.
func (h *UserHandler) SetRecoveryKeyHash(c *gin.Context) {
	externalID, err := parseUUIDParam(c.Param("id"))
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "invalid user id"})
		return
	...
	var req struct {
		RecoveryKeyHash string `json:"recovery_key_hash" binding:"required"`
	}
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "recovery_key_hash is required"})
		return
	}

	if err := h.svc.SetRecoveryKeyHash(c.Request.Context(), externalID, req.RecoveryKeyHash); err != nil {
		if err == repository.ErrNotFound {
			c.JSON(http.StatusNotFound, gin.H{"error": "user not found"})
		} else {
			h.logger.Error("SetRecoveryKeyHash failed", zap.Error(err))
			c.JSON(http.StatusInternalServerError, gin.H{"error": "internal error"})
		}
		return
	}
