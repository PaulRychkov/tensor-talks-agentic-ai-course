...
- **Agent Service (agent-service)**  
  AI-сервис на Python для проведения интервью с использованием LLM:
  - реализует мультиагентную архитектуру на базе LangChain и LangGraph;
  - агент-интервьюер проводит диалог, задает вопросы, дает уточнения и подсказки;
  - агент-аналитик формирует структурированный отчет с оценкой ответов;
  - читает события из Kafka и генерирует ответы;
  - получает программу интервью от session-service;
...
- **Kafka**  
  ...
  - `tensor-talks-chat.events.out` — события от BFF к модели (старт чата, сообщения пользователя);
  - `tensor-talks-chat.events.in` — события от модели к BFF (вопросы, результаты, завершение);
  - `tensor-talks-interview.build.request` — запрос на создание программы интервью;
  - `tensor-talks-interview.build.response` — ответ с готовой программой интервью;
...
`backend/bff-service`, `backend/session-service`, `backend/agent-service`, 
...
- [LOCAL_SETUP_MANUAL.md](LOCAL_SETUP_MANUAL.md) - локальная разработка
- [CLOUD_DEPLOYMENT_MANUAL.md](CLOUD_DEPLOYMENT_MANUAL.md) - развертывание в облаке
- [DEVOPS.md](DEVOPS.md) - DevOps архитектура и CI/CD пайплайн
...
**Текущий этап**: MVP готов, agent-service реализован, идет пилотная апробация
...
- [x] AI-агент для проведения интервью (agent-service на LangChain/LangGraph)
...
- [INSTALLATION_MANUAL.md](INSTALLATION_MANUAL.md) - **Полное руководство: установка, локальная разработка, облачное развертывание**
...
- [helm/README.md](helm/README.md) - Helm chart документация
...
- [backend/KAFKA.md](backend/KAFKA.md) - Kafka документация
- [backend/METRICS.md](backend/METRICS.md) - Метрики
