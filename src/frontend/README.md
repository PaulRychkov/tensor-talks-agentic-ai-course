...
- не обращается напрямую ни к `auth-service`, ни к `user-crud-service`, ни к БД.
...
- фронтенд никогда не ходит напрямую в `auth-service`/`user-crud-service` — только в `bff-service`;
...
- BFF дальше проксирует запросы в `auth-service`, который уже работает с `user-crud-service`.
