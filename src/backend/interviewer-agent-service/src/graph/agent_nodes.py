...
def _build_system_prompt(state: AgentState) -> str:
    ...
    mode_policy = {
        ...
        "study": (
            "Режим STUDY (обучение). Ведёшь как преподаватель. "
            f"Лимит уточнений на вопрос: {max_interactions}. "
            "Если студент не знает / ответ пустой — "
            "объясни пропущенные аспекты и переходи к следующему вопросу."
        ),
    }.get(session_mode, "")
...
Типичная последовательность для ответа пользователя:
  ...
  3. emit_response — ФИНАЛЬНЫЙ шаг: отправь текст пользователю и объяви action
     ...
     ВАЖНО при action=next_question: message ДОЛЖЕН содержать и краткий комментарий к ответу, И текст СЛЕДУЮЩЕГО вопроса. Не пиши «переходим дальше» без самого вопроса.
