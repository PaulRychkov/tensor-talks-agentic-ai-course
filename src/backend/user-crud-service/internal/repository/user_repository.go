...
type UserRepository interface {
	...
	// SetRecoveryKeyHash обновляет bcrypt-хеш ключа восстановления (§10.10).
	SetRecoveryKeyHash(ctx context.Context, externalID uuid.UUID, hash string) error
	// Login word dictionary methods (§10.10)
	GetActiveAdjectives(ctx context.Context) ([]models.LoginAdjective, error)
	GetActiveNouns(ctx context.Context) ([]models.LoginNoun, error)
}
...
// SetRecoveryKeyHash обновляет bcrypt-хеш ключа восстановления для пользователя.
func (r *GormUserRepository) SetRecoveryKeyHash(ctx context.Context, externalID uuid.UUID, hash string) error {
	result := r.db.WithContext(ctx).Model(&models.User{}).Where("external_id = ?", externalID).Update("recovery_key_hash", hash)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected == 0 {
...
// ── Login word dictionary (§10.10) ───────────────────────────────────────────

// GetActiveAdjectives returns all active adjectives ordered alphabetically.
func (r *GormUserRepository) GetActiveAdjectives(ctx context.Context) ([]models.LoginAdjective, error) {
	var words []models.LoginAdjective
	if err := r.db.WithContext(ctx).Where("is_active = true").Order("word").Find(&words).Error; err != nil {
		return nil, err
	}
	return words, nil
}

// GetActiveNouns returns all active nouns ordered alphabetically.
func (r *GormUserRepository) GetActiveNouns(ctx context.Context) ([]models.LoginNoun, error) {
	var words []models.LoginNoun
	if err := r.db.WithContext(ctx).Where("is_active = true").Order("word").Find(&words).Error; err != nil {
		return nil, err
	}
	return words, nil
