"""Kafka consumer for session.completed events"""
...
class KafkaConsumer:
    """Kafka consumer wrapper for session.completed events"""
    ...
    def poll(self, timeout: float = 1.0) -> List[KafkaEvent]:
        ...
        try:
            ...
            self.metrics.error_count.labels(
                error_type="kafka_consumer_exception",
                service=settings.service_name,
            ).inc()
            ...
            try:
                while self.running:
                    try:
                        ...
                        for event in events:
                            if self.handler and event.event_type == EventType.SESSION_COMPLETED:
