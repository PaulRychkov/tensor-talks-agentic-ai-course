...
// SessionMeta содержит метаданные сессии (kind, topics, level).
...
// GetInterviewProgramWithMeta получает программу интервью + метаданные сессии.
func (c *SessionManagerClient) GetInterviewProgramWithMeta(ctx context.Context, sessionID uuid.UUID) (*models.InterviewProgram, *SessionMeta, error) {
	resp, err := c.fetchProgram(ctx, sessionID)
	if err != nil {
		return nil, nil, err
	}
	meta := &SessionMeta{
		SessionKind: resp.SessionMode,
		Topics:      resp.Topics,
		Level:       resp.Level,
	}
	return &resp.Program, meta, nil
}
...
func (c *SessionManagerClient) fetchProgram(ctx context.Context, sessionID uuid.UUID) (*GetInterviewProgramResponse, error) {
