...
class SessionServiceClient:
    ...
    async def get_program_with_mode(self, session_id: str) -> Tuple[Optional[Dict[str, Any]], str]:
        """Get interview program and session mode. Returns (program, mode)."""
        try:
            response = await self.client.get(
                f"{self.base_url}/sessions/{session_id}/program"
            )
            response.raise_for_status()
            data = response.json()
            return data.get("program"), data.get("session_mode", "interview")
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                logger.warning(
                    "Program not found for session",
            ...
            logger.error("Failed to get program with mode", session_id=session_id, error=str(e))
            raise
